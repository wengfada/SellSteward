...
//    private static Log log = LogFactory.getLog(OAuthV2.class);// 日志输出
    ...
    public List<NameValuePair> getTokenParamsList() {
        List<NameValuePair> paramsList = new ArrayList<NameValuePair>();
            ...
            Logger.d("oauth_consumer_key",clientId);
            Logger.d("access_token",accessToken);
            Logger.d("openid",  openid);
            Logger.d("clientip", clientIP);
            Logger.d("oauth_version",  oauthVersion);
	...
	public String toString() {
		return "OAuthV2 [redirectUri=" + redirectUri + ", clientId=" + clientId
				+ ", clientSecret=" + clientSecret + ", responseType="
				+ responseType + ", type=" + type + ", authorizeCode="
				+ authorizeCode + ", accessToken=" + accessToken
				+ ", expiresIn=" + expiresIn + ", grantType=" + grantType
